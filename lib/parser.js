...
function verifyTitleMatch(filename, searchTitles) {
    // Fail CLOSED on an empty/missing allow-list. Returning true here meant that
    // whenever every candidate title was filtered out for being too short, the
    // relevance check passed for *everything* — including unrelated provider
    // results. Callers already log drops, so a closed failure is visible.
    if (!searchTitles || searchTitles.length === 0) return false;
